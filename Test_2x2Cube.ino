...
void setup() {
  ...
  for(int i = 0; i<3; i++) // 전원핀은 출력이므로 OUTPUT으로 설정한다.
...
void loop() {
  ...
  if(digitalRead(pin_btn) == LOW) // 버튼이 누르지 않았을 때 상태가 1, 버튼을 누르면 상태가 0 이다.
  {
  ...
  for(int i = 0; i < 6; i++) // 총 6면이므로 0부터 5까지 반복한다.
  {
    ...
    digitalWrite(pin_power[i],HIGH); // 전원핀에 HIGH를 줌으로써 전위차가 발생하여 전류가 흐른다.
